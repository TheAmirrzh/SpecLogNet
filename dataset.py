...
def fixed_collate_fn(batch_list: List[Data]) -> Batch:
    ...
    for i, data in enumerate(batch_list):
        ...
        # Increment the target index 'y' by the current offset
    ...
    try:
        ...
        # --- FIX: Use the pre-computed 'node_offsets_list' ---
    ...
    # Store metadata
    if hasattr(batch_list[0], 'metadata_dict'):
        batch.meta_list = [data.metadata_dict for data in batch_list]
...
# ✅ FIXED:
# In dataset.py, replace the old function with this one:

def compute_applicable_rules_for_step(nodes, edges, step_idx, proof_steps, id2idx):
    """
    FIXED: Reconstructs the exact proof state by trusting the proof_steps list.
    Computes applicability based on known facts from steps 0 to step_idx-1.
    """
    known_atoms = set()
    
    # Step 1: Add all initial axioms (facts)
    for node in nodes:
        if node.get("is_initial", False) and node["type"] == "fact":
            known_atoms.add(node.get("atom"))
    
    # Step 2: Add all derived facts from the ground-truth proof history
    # We only look at steps *before* the current step_idx.
    for i in range(step_idx):
        prev_step = proof_steps[i]
        derived_nid = prev_step.get('derived_node')
        
        # Find the node corresponding to this NID
        if derived_nid in id2idx:
            derived_node_idx = id2idx[derived_nid]
            derived_node = nodes[derived_node_idx]
            known_atoms.add(derived_node.get("atom"))
        # else: This would be a data integrity error
            
    # Step 3: Now, compute applicability based on this *correct* known_atoms set
    ...
    for i, node in enumerate(nodes):
        if node["type"] == "rule":
            ...
            # 1. All its premises (body) are in the known_atoms set.
            # 2. Its conclusion (head) is NOT *already* in the known_atoms set.
            is_applicable = body_atoms.issubset(known_atoms) and (head_atom not in known_atoms)
...
class ProofStepDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> 'Data':
        ...
        try:
            ...
            goal_nid = metadata.get("goal_nid", -1)
            goal_distances = self._get_goal_distances(num_nodes, edges, goal_nid, id2idx)
            
            # Normalize distances
            max_d = goal_distances.max()
            if max_d == 0 or max_d == num_nodes + 1:
                norm_goal_distances = torch.zeros_like(goal_distances)
            else:
                norm_goal_distances = goal_distances / max_d
                
            # We need to map rule_node distance to their head_atom distance
            goal_dist_feature = torch.zeros_like(norm_goal_distances)
            
            # Find rule heads
            rule_head_map = {} # map rule_idx -> head_fact_idx
            for e in edges:
                if e["etype"] == "head":
                    src_idx = id2idx.get(e["src"], -1)
                    dst_idx = id2idx.get(e["dst"], -1)
                    if src_idx != -1 and dst_idx != -1:
                        rule_head_map[src_idx] = dst_idx
                        
            for i, node in enumerate(nodes):
                if node["type"] == "fact":
                    goal_dist_feature[i] = norm_goal_distances[i]
                elif node["type"] == "rule":
                    head_idx = rule_head_map.get(i, -1)
                    if head_idx != -1:
                        goal_dist_feature[i] = norm_goal_distances[head_idx]
                    else:
                        # Rule with no head? Set to max distance
                        goal_dist_feature[i] = 1.0
            
            goal_dist_feature = goal_dist_feature.unsqueeze(1) # [N, 1]
            ...
            pyg_data.derived_mask = compute_derived_mask(
                {'num_nodes': num_nodes, 'derivations': [
                    ...
                    for i, step_data in enumerate(proof_steps[:step_idx], start=1)
            ...
            pyg_data.eig_mask = eig_mask.contiguous()
             ...
             # Concatenate base features with the current temporal state

            # Normalize step_numbers to [0, 1] (prevents large inputs to GNN)
            proof_length = max(metadata.get('proof_length', len(proof_steps)), 1)
            # Clamp step_numbers to avoid division by zero or invalid values if proof_length is 0
            if proof_length == 0:
                proof_length = 1

            norm_step_numbers = pyg_data.step_numbers.float() / proof_length

            # Overwrite x with the new, richer feature tensor
            pyg_data.x = torch.cat([
                x_base,                                     # [N, 22]
                pyg_data.derived_mask.unsqueeze(1).float(), # [N, 1]
                norm_step_numbers.unsqueeze(1),
                goal_dist_feature.to(x_base.device)              # [N, 1]
            ], dim=1)


            # ===== STEP 12: Add metadata (for curriculum) =====
            pyg_data.metadata_dict = {
    ...
    def _compute_node_features(self, inst: Dict, step_idx: int, known_atoms_set: set, id2idx: dict) -> torch.Tensor:
        ...
        return feats
    def _get_goal_distances(self, num_nodes: int, edges: List[Dict], 
                            goal_nid: int, id2idx: dict) -> torch.Tensor:
        """
        Computes shortest path distance from all nodes to the goal_nid
        using BFS on the *undirected* graph.
        """
        ...
        for edge in edges:
            ...
            if src_idx != -1 and dst_idx != -1:
                ...
                adj[dst_idx].append(src_idx) # Undirected
        
        # Max distance for unconnected nodes
        max_dist = num_nodes + 1 
        distances = torch.full((num_nodes,), max_dist, dtype=torch.float)
        
        goal_idx = id2idx.get(goal_nid, -1)
        
        if goal_idx == -1:
            # No goal, return max distance
            return distances
        
        # BFS from the goal
        q = deque([(goal_idx, 0)]) # (node_idx, distance)
        visited = {goal_idx}
